...
public class AlertManager {
    ...
    // Key: "playerUUID:MATERIAL" — per-player per-ore cooldown
    ...
    public void fireAlert(Player player, int count, Material mostMinedOre, Location location) {
        OreConfig cfg = plugin.getOreTracker().getOreConfig(mostMinedOre);
        if (cfg == null) return;

        long now = System.currentTimeMillis();
        // Per-player, per-ore cooldown key
        String cooldownKey = player.getUniqueId().toString() + ":" + mostMinedOre.name();
        Long last = lastAlertTime.get(cooldownKey);
        if (last != null && (now - last) < cfg.alertCooldownMillis) return;
        ...
        long seconds = cfg.timeWindowMillis / 1000;
        ...
        if (discordEnabled && webhookUrl != null && !webhookUrl.isEmpty() && !webhookUrl.contains("YOUR_WEBHOOK")) {
            ...
            final long    fSecs   = seconds;
            final long    fNow    = now;
            Bukkit.getScheduler().runTaskAsynchronously(javaPlugin,
                    () -> sendDiscordWebhook(pName, fCount, fOre, fLoc, fWorld, fSecs, fNow));
        }
    }
    ...
    void sendDiscordWebhook(String playerName, int count, Material ore,
        ...
        try {
            ...
            String countV = plugin.getLang().get("discord.embed-count-value",
            ...
            String json = "{\"content\":\"" + escapeJson(mention) + "\","
                    ...
                    + field(plugin.getLang().get("discord.embed-count"), countV)
